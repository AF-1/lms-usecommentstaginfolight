...
package Plugins::UseCommentsTagInfoLight::Settings;
...
my $log = logger('plugin.usecommentstaginfolight');
...
sub name {
	return Slim::Web::HTTP::CSRF->protectName('PLUGIN_USECOMMENTSTAGINFOLIGHT');
}

sub page {
	return Slim::Web::HTTP::CSRF->protectURI('plugins/UseCommentsTagInfoLight/settings/settings.html');
}
...
sub handler {
	...
	if ($paramRef->{saveSettings}) {
		...
		$prefs->set('configmatrix', \%configmatrix);
		$paramRef->{configmatrix} = \%configmatrix;
		$log->debug('SAVED VALUES = '.Dumper(\%configmatrix));
	...
	foreach my $thisconfig (sort keys %{$configmatrix}) {
		$log->debug('thisconfig = '.$thisconfig);
	...
	$paramRef->{configmatrix} = \@thisconfiglistsorted;
	$paramRef->{itemcount} = scalar @thisconfiglistsorted;
	$log->debug('page list = '.Dumper($paramRef->{configmatrix}));
